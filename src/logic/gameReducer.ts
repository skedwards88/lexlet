...
import type {GameReducerPayload, GameState} from "../Types";
...
export function gameReducer(
  ...
  if (payload.action === "startWord") {
    ...
    const newHints = cloneDeep(currentGameState.hints);
    ...
    if (newHints[payload.clueIndex].every((i) => i)) {
      const newClueMatches = cloneDeep(currentGameState.clueMatches);
    ...
    console.log(`unknown action: ${(payload as {action: string}).action}`);
    return {...currentGameState};
  }
}
