...
function tallyItems(items: string[] = []): Record<string, number> {
...
export function getPlayableBoard({
  ...
  numClues: number;
  seed: string;
}): {letters: Letter[]; colors: PrimaryColor[]; clueIndexes: number[][]} {
  const colorDistribution = ["red", "blue", "yellow"] as PrimaryColor[];
  let foundPlayableBoard = false;
  let letters = [] as Letter[];
  let colors = [] as PrimaryColor[];
  let clueIndexes = [];
  ...
  while (!foundPlayableBoard) {
    ...
    let colorTally: Record<string, number> = {};
    while (
